...
if "current_chat_id" not in st.session_state:
    ...
    st.session_state.all_chats[new_id] = {
        "title": "New Chat",
        "messages": [
            {"role": "assistant", "content": "Hello! I am OmniMind Assistant. Ask me anything, attach files, or ask me to generate images!"}
        ]
    }
...
def is_image_request(prompt):
    triggers = ["generate an image", "create an image", "draw an image", "make an image", "generate image", "create image", "draw", "picture of"]
...
with st.sidebar:
    ...
    if st.button("➕ New chat", use_container_width=True, type="primary"):
        ...
        st.session_state.all_chats[new_id] = {
            "title": "New Chat",
            "messages": [
                {"role": "assistant", "content": "Hello! How can I help you in this new session?"}
            ]
        }
        st.session_state.current_chat_id = new_id
        st.session_state.file_context = ""
...
if prompt := st.chat_input("Ask anything, generate an image, or inquire about uploaded files..."):
    ...
    if is_image_request(prompt):
        ...
        if st.session_state.file_context:
            api_messages.append({
                "role": "system",
                "content": f"You are OmniMind Assistant. Use this attached document context if helpful:\n\n{st.session_state.file_context[:10000]}"
            })
